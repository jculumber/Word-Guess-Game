...
var remainingGuesses = 10;  //Number of chances the player has to figure out word
...
//This function is run whenever the user presses a key
    document.onkeyup = function(event) {
        ...
        for (k = 0; k < currentWordLetters.length; k++) {
            if (currentWordLetters.indexOf[playerGuess] == playerGuess) {
                guessingWord.push(playerGuess);
        ...
        if ((lettersGuessed.length == currentWord.length) && (guessingWord.indexOf(_) == -1)) {
            alert("You win! Great job! Click 'OK' to start a new game");
            wins++;
            newGame();
        };
...
//Begin
    window.onload = pageLoad(), newGame();
//Reset game
...
var aryWords = ["dice",...];
var aryLetters = aryWords[randomNumber]; //["d," "i", "c", "e",]
var aryShadow = [];
for (i = 0; i < aryLetters.length;i++) {
    aryShadow.push("_");
}

keyUp (calculations) listener {
    var letter = event.key.toLowerCase(); // "a"
    for (i = 0; i < aryLetters[i] == letter) {
        aryShadow[i] = letter; // ["_", "i", "_", "_"];
...
still needs:
calculations
load letter into guess[];
display

if remaining guesses = 0 and there is still an underscore, player loses
if remaining guesses >= 0, and there is no underscore, player wins
*/